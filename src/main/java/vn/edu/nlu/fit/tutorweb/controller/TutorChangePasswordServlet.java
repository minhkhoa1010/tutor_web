...
@WebServlet("/tutor/change-password")
public class TutorChangePasswordServlet extends HttpServlet {
    private final UserAuthDAO authDAO = new UserAuthDAO();

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        // Chuyển hướng sang thư mục views/tutor
        request.getRequestDispatcher("/views/tutor/change-password.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!newPassword.equals(confirmPassword)) {
            ...
            response.sendRedirect(request.getContextPath() + "/tutor/change-password");
            return;
        }

        // 2. Xác thực mật khẩu cũ bằng cách dùng lại hàm loginWithCredentials có sẵn
