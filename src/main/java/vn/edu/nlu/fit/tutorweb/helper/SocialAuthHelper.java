...
public class SocialAuthHelper {
    ...
    public static JsonObject getGoogleUserInfo(String code) throws IOException {
        ...
        String response = Request.Post("https://oauth2.googleapis.com/token")
                .bodyForm(Form.form()
                        .add("client_id", DBProperties.get("google.client.id"))
                        .add("client_secret", DBProperties.get("google.client.secret"))
                        .add("redirect_uri", DBProperties.get("google.redirect.uri"))
                        .add("code", code)
    ...
    public static JsonObject getFacebookUserInfo(String code) throws IOException {
        // 1. Đổi "code" lấy Access Token từ Facebook Graph API
        String tokenUrl = "https://graph.facebook.com/v19.0/oauth/access_token?"
                + "client_id=" + DBProperties.get("facebook.client.id")
                + "&redirect_uri=" + DBProperties.get("facebook.redirect.uri")
                + "&client_secret=" + DBProperties.get("facebook.client.secret")
                + "&code=" + code;
